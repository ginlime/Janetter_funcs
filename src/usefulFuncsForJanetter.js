...
// 　profile や config と違い、notice は見えていないが常時動作しているため、main で値を変えただけだと、終了時に notice 側に残っている古い値が設定されてしまうことがある。
// ※同様に、syncConfig を使用するプラグインでは、プラグイン冒頭で if(_Janetter_Window_Type != "main") return; などとして特定のウィンドウタイプを**排除してはいけない**。syncConfig の設定値の同期が notice に反映されずに古い値が設定されてしまうことがある。
